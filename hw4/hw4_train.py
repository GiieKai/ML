
# coding: utf-8

# In[20]:


import pandas as pd 
import numpy as np
from keras.preprocessing.text import Tokenizer,text_to_word_sequence
from keras.preprocessing import sequence
import pickle
#About model package
import gensim
import matplotlib.pyplot as plt
from keras import regularizers
from keras.callbacks import EarlyStopping, ModelCheckpoint
from keras.layers.core import Dense, Dropout, Activation,Flatten
from keras.layers import LSTM
from keras.layers.embeddings import Embedding
from keras.models import Model, load_model,Sequential
from keras import regularizers,initializers
from keras.callbacks import EarlyStopping
from keras.optimizers import SGD, Adam, Adadelta
from keras.layers.convolutional import Conv1D,MaxPooling1D
from gensim.models import word2vec,Word2Vec
from keras.layers.wrappers import Bidirectional
# In[2]:


#deal with label data
def dataload_label(label_path):
    data = []
    train_label = []
    with open(label_path,encoding="utf-8", errors="strict") as inputfile:
        for line in inputfile:
            train_label.append(float(line.strip().split('+++$+++')[0]))
            data.append(line.strip().split('+++$+++')[1])
    return data,train_label


# # In[3]:


#deal with no label data
def dataload_nolabel(no_label_path):
    data = []
    #train_label = []
    with open(no_label_path,encoding="utf-8", errors="strict") as inputfile:
        for line in inputfile:
            data.append(line.strip())
    return data


# # In[4]:


#deal with test data
def dataload_test(inputpath):
    test_text = [line.strip().split(',') for line in open(inputpath, encoding = 'utf-8-sig')]
    data = []
    for row in range(1,len(test_text)):
        data.append(str(test_text[row][1::]))
    return data


# # In[13]:


def textprcoessingforword2vec(input_data):
    sentences = []
    for i in range(len(input_data)):
        sentences.append(text_to_word_sequence(input_data[i],filters='\t\n'+"'",split=" "))
    return sentences


def modeltoword2vec(sentences,word_dim):
    model = Word2Vec(sentences,sg=1, size=word_dim, window=5, min_count=1)
    model.save('word2vec_no')

def wordtransformLstm(data_list,name= '',path = 'word2vec_no'):
    print("dealing with ----" + name + "---- wordtransformLstm")
    model = Word2Vec.load(path)
    data = data_list
    X =np.array([model[s] for s in data])
    #X_final = reshape_zero(X,word_dim=100,max_len_seq=40)

    return  X 

def reshape_zero(data,word_dim=100,max_len_seq=40):
    print("doing padding zero")
    return np.array([np.concatenate([np.array([0.0]*word_dim*(max_len_seq-row.shape[0])).reshape(max_len_seq-row.shape[0],word_dim),row]) for row in data])

def save_transform_npy(data,name = ''):
    print("save "+name + "in npy")
    np.save(name+'.npy',data)
    
def findmaxlen(data):
    Highest=max([len(x) for x in data])
    return Highest
    
# #deal with text and padding 
# # def textprcoessing(train_data,test_data,most_common_words):
# #     #Arguments:num_words: None or int. Maximum number of words to work with 
# #     #(if set, tokenization will be restricted to the top num_words most common words in the dataset).

# #     tokenizer = Tokenizer(num_words=most_common_words,filters='!"#$%&()*+,-./:;<=>?@[\]^_`{|}~\t\n'+"'", split=" ")
    
# #     tokenizer.fit_on_texts(train_data+test_data)
# #     vocab_size = len(tokenizer.word_index) + 1

# #     x_train_seq = tokenizer.texts_to_sequences(train_data)
# #     x_test_seq = tokenizer.texts_to_sequences(test_data)
    
# #     maxlen_x_train = len(max(x_train_seq,key=len))
    
# #     final_x_train_seq = sequence.pad_sequences(x_train_seq, maxlen=maxlen_x_train, dtype='int32',padding='pre', truncating='pre', value=0.)
# #     final_x_test_seq =sequence.pad_sequences(x_test_seq, maxlen=maxlen_x_train, dtype='int32',padding='pre', truncating='pre', value=0.)

# #     return final_x_train_seq,final_x_test_seq,maxlen_x_train,vocab_size

# # In[6]:


# #model arch.
# # def build_model(output_dim,most_common_words,max_seq_len,opt):

# #     model = Sequential()
# #     model.add(Embedding(output_dim=output_dim,
# #                         input_dim=most_common_words, 
# #                         input_length=None,embeddings_initializer = 'RandomNormal'))
# #     model.add(LSTM(128, dropout=0.5, recurrent_dropout=0.3))
# #     model.add(Dense(256, activation='relu'))
# #     model.add(Dropout(0.4))

# #     model.add(Dense(128, activation='relu'))
# #     model.add(Dropout(0.5))
# #     model.add(Dense(1, activation='sigmoid'))
# #     model.compile(loss = 'binary_crossentropy', optimizer = opt, metrics = ['accuracy'])
# #     model.summary()
# #     return model

def build_model(word2vec_dim,max_len_seq):
    model = Sequential()
    #input_shape=(max_len_seq,word_dim)
    model.add(Bidirectional(LSTM(256, input_shape=(max_len_seq,word2vec_dim),dropout=0.5, recurrent_dropout=0.3)))
    model.add(Dense(256, activation='relu'))
    model.add(Dropout(0.4))
    model.add(Dense(128, activation='relu',kernel_regularizer=regularizers.l2(0.1)))
    model.add(Dropout(0.5))
    model.add(Dense(1, activation='sigmoid'))
    model.compile(loss = 'binary_crossentropy',optimizer='Adam', metrics=['accuracy'])
    model.summary()
    return model



# # In[7]:



# #plot
# import matplotlib.pyplot as plt

# def plot_history(train_result):
#     # summarize history for accuracy
#     plt.plot(train_result.history['acc'])
#     plt.plot(train_result.history['val_acc'])
#     plt.title('model accuracy')
#     plt.ylabel('accuracy')
#     plt.xlabel('epoch')
#     plt.legend(['train', 'test'], loc='lower right')
#     plt.show()


# # In[14]:


# #--------------------run------------------------








#dim is wrong 
#test mex len is 39 but only have 38






# #information in the textprcoessing  return final_x_train_seq,final_x_test_seq,maxlen,vocab_size

# # most_common_words = 8787
# # x_train,x_test,max_seq_len,vocab_size = textprcoessing(x_load_train,x_load_test,most_common_words)


# # In[ ]:

# # opt = SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)
# # opt = Adam(lr=1e-3)
# #opt = Adadelta(lr=0.1, rho=0.95, epsilon=1e-08)

# #checkpoint = ModelCheckpoint(path+'models/model_acc{val_acc:.4f}_epoch{epoch:03d}.hdf5', monitor='val_acc', verbose=0,save_best_only=False, save_weights_only=False, mode='max', period=1)
# #model = build_model(
# #train_result = model.fit(x_train,train_label,epochs=5,batch_size=256 ,shuffle='True', validation_split = 0.1,class_weight='auto')

# #plot_history(train_result)

#model = load_model('./model/model_acc.h5')


# In[ ]:

import matplotlib.pyplot as plt


# write this thing, if i want to import this py ,  i dont write this i will run all 
if __name__ == '__main__':

    label_path =sys.arg[1]
    no_label_path = sys.arg[2]
    # test_path = './data/testing_data.txt'
    x_load_train,train_label = dataload_label(label_path)
    x_load_nolabel = dataload_nolabel(no_label_path)
    # x_load_test =  dataload_test(test_path)

    x_train_label = textprcoessingforword2vec(x_load_train)
    x_train_nolabel = textprcoessingforword2vec(x_load_nolabel)
    # x_test = textprcoessingforword2vec(x_load_test)

    # data_seq = x_train_label+ x_train_nolabel+ x_test
    # print(len(data_seq))


    word2vec_dim = 128
    max_len_seq = 43
#-------trainng word2vec------
    # modeltoword2vec(data_seq,word2vec_dim)

    X_Train_temp = wordtransformLstm(x_train_label,"train")
    # X_Test_temp = wordtransformLstm(x_test,"test")
    


#every len is different, so need to transform
    # max_len_seq_x_train_label = findmaxlen(x_train_label)
    # max_len_seq_x_train_nolabel = findmaxlen(x_train_nolabel)
    # max_len_seq_x_test = findmaxlen(x_test)


    X_Train = reshape_zero(X_Train_temp,word2vec_dim,max_len_seq)
    save_transform_npy(X_train,"train")


    # X_Test = reshape_zero(X_Test_temp,word2vec_dim,max_len_seq_x_test)
    # save_transform_npy(X_Test,"test")

#train dim as same train moedl,because LSTM need to same input shape
    
    # intoLstm = np.zeros((200000,max_len_seq_x_train_label,word2vec_dim))
    
    # for i in range(200000):
    #     for j in range(max_len_seq_x_train_label-1,-1,-1):
    #         intoLstm[i][j]=X_Test[i][j]

    model = build_model(word2vec_dim,max_len_seq)
    #ModelCheckpoint
    # https://faroit.github.io/keras-docs/2.0.0/callbacks/ 
    checkpoint = ModelCheckpoint('model_weight.h5', monitor='val_acc', verbose=1, save_best_only=True, mode='max')
    X_train = np.load("train.npy")
    train_history = model.fit(X_train, train_label,batch_size=1024, epochs=100,callbacks=[checkpoint], validation_split=0.1,class_weight='auto')

    # # summarize history for accuracy
    # plt.plot(train_history.history['acc'])
    # plt.plot(train_history.history['val_acc'])
    # plt.title('model accuracy')
    # plt.ylabel('accuracy')
    # plt.xlabel('epoch')
    # plt.legend(['train', 'test'], loc='lower right')
    # plt.show()
    # plt.savefig('train_history_uncomma.png')

    model = load_model('model_weight.h5')

    # print("--------------predict--------------")
    # X_Test = np.load("test.npy")

    # #X_test  =intoLstm
    # y_test = model.predict_classes(intoLstm)


    # Out_path = 'predict.csv'
    # with open(Out_path, 'w') as f:
    #     f.write('id,label\n')
    #     for i, v in  enumerate(y_test):
    #         f.write('%d,%d\n' %(i, v))